...
main (int argc, char * argv[])
{
   ...
   time=123456;
   while (((TF_STATUS&(BSY|DRDY|SKC|DRQ))!=(DRDY|SKC|DRQ))&&time)
      {
         if (TF_STATUS&(ERR|DWF)) puts ("Error or Drive write fault");
         time--;
      };
   if (time==0) puts ("Drive didn't respond");

   for (i=0;i<256;i++) buf[i]=TF_DATA; /* read data */
   if (TF_STATUS&ERR) puts ("Drive error after reading data");
   ...
   if (buf[49]&(1<<9)) puts ("yes"); else puts ("no");
   ...
   if (argc>2)
      if ( (*argv[1]=='t')&&(*(argv[1]+1)=='o') )
         if ( (outfile=fopen(argv[2],"wb")) != 0 /*NULL*/ )
            {fwrite (buf,2,256,outfile); fclose(outfile);}
         else printf ("Could not open file %s for write\n",argv[2]);

   if (TF_STATUS&DRQ) puts ("Funny, still Data Request");
   return 0;
}
