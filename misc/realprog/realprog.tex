...
% LaTeX'd by Thomas Baetzler (BAETZLER@IRAV17.IRA.UKA.DE) 23/11/29

\parskip 1.5ex

\begin{document}

% Newsgroups: eunet.jokes,rec.humor,rec.humor.d
% From: krus@cs.concordia.ca (Mike Krus)
% Subject: Re: Real Programmers don't Eat Quiche
% Date: Mon, 19 Oct 1992 15:09:07 GMT
% 
...
\author{Ed Post\thanks{email: (decvax $|$ ucbvax $|$ cbosg $|$ pur-ee $|$ lbl-unix)!teklabs!iddic!evp}\\Tektronix, Inc.\\P.O. Box 1000 m/s 63-205\\Wilsonville, OR
97070\\Copyright \copyright 1982}
\maketitle
...
were the ones that didn't.  A real computer programmer said things like
``{\tt DO 10 I=1,10}'' and ``{\tt ABEND}'' (they actually talked in capital
...
--- they're so impersonal''.  (A previous work\footnote{Feirstein, B., {\sl
Real Men Don't Eat Quiche}, New York, Pocket Books, 1982.} points out that
Real Men don't ``relate'' to anything, and aren't afraid of being
impersonal.)
...
\item Real Programmers aren't afraid to use {\tt GOTO}s.

\item Real Programmers can write five page long {\tt DO} loops without
getting confused.

\item Real Programmers like Arithmetic {\tt IF} statements --- they make
the code more interesting.

\item Real Programmers write self-modifying code, especially if they can
save 20 nanoseconds in the middle of a tight loop.

\item Real Programmers don't need comments --- the code is obvious.

\item Since FORTRAN doesn't have a structured {\tt IF}, {\tt REPEAT}
$\ldots$ {\tt UNTIL}, or {\tt CASE} statement, Real Programmers don't have
to worry about not using them.  Besides, they can be simulated when
necessary using assigned {\tt GOTO}s.
...
an entire book\footnote{Wirth, N., {\sl Algorithms + Datastructures =
Programs}, Prentice Hall, 1976.} contending that you could write a program
...
Lists, Structures, Sets -- these are all special cases of arrays and can be
treated that way just as easily without messing up your programing language
with all sorts of complications.  The worst thing about fancy data types is
that you have to declare them, and Real Programming Languages, as we all
know, have implicit typing based on the first letter of the (six character)
variable name.
...
Real Programmer usually includes a keypunch and lineprinter in his toolkit,
...
Research Center for use on their Alto and Dorado computers\footnote{Xerox
PARC editors $\ldots$}.

Unfortunately, no Real Programmer would ever use a computer whose operating
system is called SmallTalk, and would certainly not talk to the computer
with a mouse.
...
transmission line noise than readable text\footnote{Finseth, C.,{\sl Theory
and Practice of Text Editors --- or --- a Cookbook for an EMACS}, B.S.
Thesis, MIT/LCS/TM-165, Massachusetts Institute of Technology, May 1980.}.
One of the more entertaining games to play with TECO is to type your name
in as a command line and try to guess what it does.  Just about any
possible typing error while talking with TECO will probably destroy your
...
security''.  Some programming tools NOT used by Real Programmers:

\begin{itemize}

\item FORTRAN preprocessors like MORTRAN and RATFOR.  The Cuisinarts of
...
\item Source code maintainance systems.  A Real Programmer keeps his code
locked up in a card file, because it implies that its owner cannot leave
his important programs unguarded\footnote{Weinberg, G., {\sl The Psychology of Computer
Programming}, New York, Van Nostrabd Reinhold, 1971, page 110.}.
...
\item Real Programmers work for Los Alamos National Laboratory, writing
atomic bomb simulations to run on Cray I supercomputers.

\item Real Programmers work for the National Security Agency, decoding
Russian transmissions.

\item It was largely due to the efforts of thousands of Real Programmers
working for NASA that our boys got to the moon and back before the
Russkies.

\item The computers in the Space Shuttle were programmed by Real
Programmers.

\item Real Programmers are at work for Boeing designing the operating
...
groundbased FORTRAN programs and small spacecraft-based assembly language
programs, they are able to do incredible feats of navigation and
improvisation --- hitting ten-kilometer wide windows at Saturn after six
years in space, repairing or bypassing damaged sensor platforms, radios,
and batteries.  Allegedly, one Real Programmer managed to tuck a
patternmatching program into a few hundred bytes of unused memory in a
...
PASCAL program (or PASCAL programmer) for navigation to these tolerances.
...
language called ``ADA'' ((r), DoD).  For a while, it seemed that ADA was
...
system and rearranging memory, and Edsgar Dijkstra doesn't like
it\footnote{Dijkstra, E., {\sl On the GREEN Language Submitted to the DoD},
Sigplan notices, Volume 3, Number 10, October 1978.}.  (Dijkstra, as I'm
sure you know, was the author of ``GoTos Considered Harmful'' --- a
landmark work in programming methodology, applauded by Pascal Programmers
...
video games at Atari, for example.  (But not playing them --- a Real
Programmer knows how to beat the machine every time:  no challange in
...
fair number people doing Graphics in order to avoid having to write COBOL
programs.
...
\item At a party, the Real Programmers are the ones in the corner talking
about operating system security and how to get around it.

\item At a football game, the Real Programmer is the one comparing the
plays against his simulations printed on 11 by 14 fanfold paper.

\item At the beach, the Real Programmer is the one drawing flowcharts in
the sand.

\item A Real Programmer goes to discos to watch the light shows.

\item At a funeral, the Real Programmer is the one saying ``Poor George.
And he almost had the sort routine working before the coronary.''

\item In a grocery store, the Real Programmer is the one who insists on
...
\item Listings of all programs the Real Programmer has ever worked on,
piled in roughly chronological order on every flat surface in the office.

\item Some half-dozen or so partly filled cups of cold coffee.
Occasionally, there will be cigarette butts floating in the coffee.  In
some cases, the cups will contain Orange Crush.

\item Unless he is very good, there will be copies of the OS JCL manual and
the Principles of Operation open to some particularly interesting pages.

\item Taped to the wall is a line-printer Snoopy calender for the year
1969.

\item Strewn about the floor are several wrappers for peanut butter filled
...
\item Hiding in the top left-hand drawer of the desk is a stash of
double-stuff Oreos for special occasions.

\item Underneath the Oreos is a flow-charting template, left there by the
...
\item A Real Programmer might or might not know his wife's name.  He does,
however, know the entire ASCII (or EBCDIC) code table.

\item Real Programmers don't know how to cook.  Grocery stores aren't open
...
with the same outlook on life as their elders.  Many of them have never
...
FORTRAN!  Are we destined to become an industry of Unix hackers and Pascal
programmers?
...
more subtle tricks, like adding structured coding constructs to FORTRAN
have failed.  Oh sure, some computer vendors have come out with FORTRAN 77
compilers, but every one of them has a way of converting itself back into a
FORTRAN 66 compiler at the drop of an option card --- to compile {\tt DO}
...
hackers ({\ }\footnote{Rose, Frank, {\sl Joy of Hacking}, Science 82,
Volume 3, Number 9, November 1982, pages 58 - 66.} and {\ }\footnote{{\sl
The Hacker Papers}, Psychology Today, August 1980.}) leaving places like
Stanford and M.I.T.  for the Real World.  From all evidence, the spirit of
Real Programming lives on in these young men and women.  As long as there
are illdefined goals, bizarre bugs, and unrealistic schedules, there will
