...
**---------------------------------------
** 
** Datatype_Search.rexx V1.2 - 14.11.96
**
** Search DTypes.guide for keywords
**
** © 1997 Heiko Schröder         e-mail: age@thepentagon.com
...
**
**----------------------------------------
** $VER: DType_Search.rexx  V1.2 (27-Feb-97)
**----------------------------------------
**
*/
...
R="0A"X
say "›32mSearchIn»DTypes.guide« V1.2 - © Heiko Schröder - 27.02.97 -›m"
...
/*
** Check for DTypes.guide in current directory 
** If DTypes.guide is found then open it, else inform the user
*/

bool1=OPEN(AMIL_file,'DTypes.guide',"R")

IF bool1 = FALSE THEN
DO
   SAY '"DTypes.guide" wasn''t found in the current directory.'
   SAY "Press »Return« to end."
   PARSE PULL Keyword
   EXIT
END
...
/*
** Create the Dtypes_Search.guide
*/

if exists("c:search") then do
   address command "c:search >T:node search @node DTypes.guide"
   address command "c:search >T:word search "d2c(34)||Keyword||d2c(34)" DTypes.guide"
end
else do
...
if wordl.0="0" then
   say "The "||Keyword||" was not found in DTypes.guide!"
else do
   bool1=OPEN(Search_file,'T:DTypes_Search.guide',W)

   WRITELN(Search_file,'@database DTypes_Search')
   WRITELN(Search_file,'@node "Main" "DTypes_Search V1.2  - 27.02.97 - by Heiko Schröder"')
   WRITELN(Search_file,'')
   ...
   do while 1
      ...
      if ist<=zeile & zeile<=next then do             /*KeyWord between two @node s*/
         ...
         DO
            ...
            AMIL_Search_Line = '@{"*" link "DTGuide:Dtypes.guide/'Node_Name'" 'Keyword_Line'} 'vor||'@{fg shine}'||KeyWord||'@{fg text}'||nach
...
ADDRESS COMMAND 'Run >NIL: <NIL:' Viewer 'T:DTypes_Search.guide'
SAY "Press »Return« to end."
PARSE PULL Keyword
ADDRESS COMMAND 'Delete T:DTypes_Search.guide QUIET'
ADDRESS COMMAND 'Delete T:node QUIET'
ADDRESS COMMAND 'Delete T:word QUIET'
ADDRESS COMMAND 'Assign DTGuide: REMOVE'
